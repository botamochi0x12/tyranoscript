(function ($) {
    ...
    */
    ...
    $.removeRemodalEvents = function (includes_closed) {
        throw Error("`removeRemodalEvents` is deprecated. Don't use it!");
    };
     ...
     *
     * - [x] モーダルに表示される文字列を引数で変化させる
     * - [x] 「注意」画面で `OK` だけを表示させる
     * - [x] 「確認」画面で `OK` と `NG` を表示させる
     * - [x] モーダル外をクリックするとキャンセルする
     * - [ ] タブキーによる遷移をモーダルが表示されるまでできないようにする
     * - [ ] アニメーションを付与する
     */
    $.remodalCommon = function (options = {}) {
        Swal.fire({
            ...
            .then(res => {
                ...
                if (options.type !== "confirm") return;
                if (res.isConfirmed && (typeof options.on_ok === "function")) {
                    return options.on_ok();
                    // TODO: 暗転
                }
        ...
        // オープンアニメーション
        //
        // TODO: オープン開始時にアニメクラスを付ける, オープン完了時に外す
    };
